...
Private Const OFFSET_4 = 4294967295#
...
'In AidDecoder this is App.path only
        ProcessDir = App.Path & "\com0com"
        ret = CreateProcessA(cmd$, cmdline$, 0&, 0&, 1&, _
...
'Disable UAC
            ...
            WriteLog "UAC has been disabled"
        End If
'Disable New Hardware prompt
        ...
        WriteLog "New Hardware Prompt has been disabled"
...
're-set UAC to original value UAC
        ...
        WriteLog "New Hardware Prompt has been re-enabled"
...
'Not used in NmeaRouter - needs converting to prevent overflows see NmeaRouter.SpeedTimer_Timer()
Public Function ElapsedTickCount(StartTick As Long, Optional FinishTick As Long) As Long
Dim ElapsedTick As Long
If FinishTick = 0 Then FinishTick = GetTickCount
